...
itr("Overwrite newer target %s", de=u"Jüngere Zieldatei %s überschreiben")
...
itr("to remove", de=u"zu löschen")
itr("to update", de="zu aktualisieren")
itr("to copy", de="zu kopieren")
itr("were removed", de=u"wurden gelöscht")
    ...
    def getStatus(self):
        if self.simulate:
            return self._statusAnalysing % (
                self.count_same, self.count_update_file, self.count_newer,
                self.count_copy_file, self.count_delete_file)
        else:
            return self._statusDoing % (self.done_same, self.done_update_file,
                                        self.done_copy_file,
...
class SyncProject(Progresser):
    ...
    def copy_dir(self, src, target):
        ...
        for fn in srcnames:
            self.copy_it(os.path.join(src, fn), os.path.join(target, fn))
