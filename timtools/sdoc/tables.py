...
colspecs = {'l': TA_LEFT, 'r': TA_RIGHT, 'c': TA_CENTER, 'j': TA_JUSTIFY}
...
##			return w
    ...
    def addColumn(self, *args, **kw):
        if self._rowIndex != -1:
            raise PdsError("Cannot addColumn() when table data has started")
...
#debug.end()
    ...
    def formatRow(self, cmdName, *params):
        if self._rowIndex == -1:
            raise PdsError("Cannot formatRow() before the first row")
        rowIndex = self._rowIndex
        if self.model.showHeaders: rowIndex += 1
        addCellFormats(self.model.dataCellFormats, cmdName, (0, rowIndex),
                       (-1, rowIndex), *params)

    def formatCell(self, cmdName, *params):
        rowIndex = self._rowIndex
        if self.model.showHeaders: rowIndex += 1
        addCellFormats(self.model.dataCellFormats, cmdName,
                       (self.currentRow.getColIndex(), rowIndex),
                       (self.currentRow.getColIndex(), rowIndex), *params)

    def formatColumn(self, cmdName, *params):
        addCellFormats(self.model.dataCellFormats, cmdName,
                       (self.currentRow.getColIndex(), 0),
                       (self.currentRow.getColIndex(), -1), *params)
...
class TablesMixin:
    ...
    def beginTable(self, columns=None, model=None, width=None, flowStyle=None):
        ...
        ti = TableInstance(self, self.getenv(), columns, model, width,
                           flowStyle)
