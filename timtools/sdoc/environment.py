...
class BaseEnvironment(Writer):
   ...
   """
    ...
    def header(self, lvl, txt, **kw):
        self.p(txt, style=self.document.stylesheet["Heading" + str(lvl)], **kw)

    def h1(self, txt, **kw):
        self.header(1, txt, **kw)

    def h2(self, txt, **kw):
        self.header(2, txt, **kw)

    def h3(self, txt, **kw):
        self.header(3, txt, **kw)
    ...
    def img(self, filename, width=None, height=None, style=None):
        assert self._txt is None
        if style is None:
            style = self.getDefaultParaStyle()
        elem = self.document.renderer.compileImage(filename, width, height,
                                                   style)
        self.toStory(elem)
    ...
    def barcode(self, txt, coding="EAN13", style=None):
        ...
        elem = self.document.renderer.compileBarcode(barCodeSymbol, style)
        self.toStory(elem)
