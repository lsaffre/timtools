    ...
    def pdftable(self, element, width, style):
        ...
        for elem in element.content:
            if elem.__class__ is html.COLGROUP:
                for col in elem.content:
                    if col.width[-1] == "*":
                        columns.append(TableColumn(weight=int(col.width[:-1])))
        ...
        for tr in html.tablerows(element, 0):  # table header
            for th in tr.content:
                col = columns[i]
                col.label = [
                    e for e in self.elem2flow(th, self.getElementStyle(th),
                                              colWidths[i])
                ]
                if len(col.label) > 0:
                    showHeaders = True
        ...
        for tr in html.tablerows(element, 2):  # table body
            ...
            for td in tr.content:
                l = []
                l += self.elem2flow(td, self.getElementStyle(td), colWidths[i])
                if len(l) == 1:
                    row.append(l[0])
                else:
                    row.append(l)
