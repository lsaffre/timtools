            ...
            for col in rpt.columns:
                for pg in range(rpt.ds.lastPage):
                    if pg != 0 \
                        ...
                        self.addChild(name=rptname(self,
                                                   rpt,
                                                   pageNum=pg + 1,
        ...
        if self.menuBar is not None:
            for menu in self.menuBar.menus:
                ...
                for mi in menu.items:
                    ...
                    wr('<li>')
                    self.writeLink(href=mi.action, label=mi.getLabel())
                    wr('</li>')
                wr('</ul>')
    ...
    def __html__(self, doc):
        ...
        if True:  # flup.lastPage > 1:
            ...
            if pageNum == 1:
                ...
                doc.writeLink(
                    href=rptname(doc, rpt, sortColumn=sortColumn, pageNum=1) +
                    doc.extension,
                    label=firstPageButton)

                doc.writeLink(href=rptname(
                    doc, rpt, sortColumn=sortColumn, pageNum=pageNum - 1) +
                              doc.extension,
...
##                           label=prevPageButton)
            ...
            if pageNum == ds.lastPage:
                ...
                doc.writeLink(href=rptname(
                    doc, rpt, sortColumn=sortColumn, pageNum=ds.lastPage) +
                              doc.extension,
...
##                           label=lastPageButton)
        ...
        for col in rpt.columns:
            ...
            if col.datacol == sortColumn:
                ...
                url = rptname(
                    doc, rpt, pageNum=pageNum,
                    sortColumn=col.datacol) + doc.extension
                doc.writeLink(href=url,
                              label=col.getLabel(),
        ...
        # iterate...
        rowno = 0
        for datarow in ds.child(pageNum=pageNum, sortColumns=[sortColumn]):
            rowno += 1
            if rowno % 2 == 0:
                wr("<tr class=''>\n")
            else:
...
class StaticHtmlDocument(HtmlDocument):
    ...
    def save(self, sess, targetRoot=".", simulate=False):
        filenames = []
        dirname = opj(targetRoot, self.location.replace("/", os.path.sep))
