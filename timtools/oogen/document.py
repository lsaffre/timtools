...
class Document:
    ...
    def createStyles(self):
        ...
        s.append(
            elements.Properties(decimalPlaces=2,
                                fontName="Arial",
                                language="en",
                                country="US",
        ...
        s = elements.CurrencyStyle(name="N106P0",
                                   ...
                                   volatile=True)
        s.append(
            elements.Number(decimalPlaces=2, minIntegerDigits=1,
                            grouping=True))
        ...
        self.styles.append(
            elements.Style(name="Default", family="table-cell", volatile=True))

        s = elements.Style(name="Result",
                           family="table-cell",
                           parentStyleName="Default")
        s.append(
            elements.Properties(fontStyle="italic",
        ...
        self.styles.append(
            elements.Style(name="Result2",
                           family="table-cell",
                           parentStyleName="Default",
                           dataStyle="N106"))

        s = elements.Style(name="Heading",
                           family="table-cell",
                           parentStyleName="Default")
        s.append(
            elements.Properties(textAlign="center",
                                textAlignSource="fix",
                                fontSize="16pt",
                                fontStyle="italic",
                                fontWeight="bold"))
        ...
        s = elements.Style(name="Heading1",
                           family="table-cell",
    ...
    def createAutoStyles(self):
        ...
        pm.append(
            elements.FootnoteSep(width="0.018cm",
                                 distanceBeforeSep="0.101cm",
                                 distanceAfterSep="0.101cm",
                                 adjustment="left",
                                 relWidth="25%",
                                 color="#000000"))

        h = elements.HeaderStyle()
        self.headerProperties = elements.Properties(minHeight="0.751cm",
                                                    marginLeft="0cm",
                                                    marginRight="0cm",
                                                    marginBottom="0.25cm")
        ...
        self.footerProperties = elements.Properties(minHeight="0.751cm",
                                                    marginLeft="0cm",
                                                    marginRight="0cm",
                                                    marginBottom="0.25cm")
        ...
        if False:
            ...
            h = elements.HeaderStyle()
            pm.append(h)
            p = elements.Properties(minHeight="0.751cm",
                                    marginLeft="0cm",
                                    marginRight="0cm",
                                    marginBottom="0.25cm",
...
##         return OoText(self,filename)
    ...
    def save(self):  # ,showOutput=False):
        ...
        for f in self.ifiles:
            zf.write(os.path.join(self.tempDir, f.filename), f.filename)
        zf.close()
        #sess.status()
...
class SpreadsheetDocument(Document):
    ...
    def par(self, *args, **kw):
        raise element.InvalidRequest("Spreadsheet body contains only tables")

    def header(self, *args, **kw):
        raise elements.InvalidRequest("Spreadsheet body contains only tables")
