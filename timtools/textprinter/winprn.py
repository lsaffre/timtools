...
charsets = {"cp850": win32con.OEM_CHARSET, "cp437": win32con.OEM_CHARSET}
...
http://newcenturycomputers.net/projects/pythonicwindowsprinting.html
    ...
    def onBeginPage(self):
        ...
        offsetx = self.dots2twips_x(
            self.dc.GetDeviceCaps(devcaps.PHYSICALOFFSETX))
        offsety = self.dots2twips_y(
            self.dc.GetDeviceCaps(devcaps.PHYSICALOFFSETY))
    ...
    def setCpi(self, cpi):
        ...
        self.session.debug("setCpi(): cpi=%d, lineWidth()=%d, cpl=%d", cpi,
                           self.lineWidth(), self.cpl)
...
#    text = text.encode(locale.getdefaultlocale()[1])
        ...
        self.session.debug("self.dc.TextOut(%d,%d,%r) %.2f cpi", int(self.x),
                           int(self.y), self.line,
